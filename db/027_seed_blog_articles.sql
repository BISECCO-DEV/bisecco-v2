-- =====================================================================
-- 027 — Seed 5 articles SEO premium pour le blog Bisecco
-- =====================================================================
-- À appliquer dans Supabase Dashboard → SQL Editor.
--
-- Articles ciblant des mots-clés à fort volume + intention commerciale :
--   1. "Combien coûte un plombier en 2026"        (volume FR : ~12k/mois)
--   2. "Comment vérifier un artisan avant signer" (volume FR : ~3k/mois)
--   3. "Devis travaux : 7 pièges à éviter"        (volume FR : ~4k/mois)
--   4. "MaPrimeRénov' 2026 : guide complet"       (volume FR : ~50k/mois)
--   5. "Maçon, menuisier ou charpentier"          (volume FR : ~2k/mois)
--
-- Chaque article :
--   - 1500-2500 mots de contenu réellement utile (pas du SEO spam)
--   - HTML structuré H2/H3/ul/blockquote
--   - Liens internes vers /metiers/[slug] et pages métier × ville
--   - Pas de mention promotionnelle agressive
-- =====================================================================

-- ─── Article 1 ─────────────────────────────────────────────────────────
INSERT INTO public.blog_posts (slug, title, excerpt, body, category, image_url, read_time, author_name, status, published_at)
VALUES (
...
<h2>⚠️ Urgence : les tarifs explosent</h2>
...
<blockquote>
  <p><strong>⚠️ Attention aux arnaques.</strong> Méfiez-vous des "plombiers d''urgence" qui annoncent "à partir de 39 €" sur des pubs Google sponsorisées. C''est presque toujours un appât. Une fois sur place, la facture grimpe à 800-2 000 €.</p>
</blockquote>
...
<h2>⚠️ Condition INDISPENSABLE : artisan RGE</h2>
...
<blockquote>
  <p><strong>⚠️ Erreur classique :</strong> commencer les travaux AVANT d''avoir reçu l''accord. Résultat : prime refusée. <strong>Attendez toujours le feu vert ANAH.</strong></p>
</blockquote>
...
-- =====================================================================
-- Vérification finale : compter les articles publiés
-- =====================================================================
SELECT COUNT(*) AS total_articles_publies FROM public.blog_posts WHERE status = 'published';
